...
![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/92cb834d86e534ceb9327162eab1dac2.png)
...
* `Mini-Transaction`(mtr)
    ...
    * `redo log 记录结构`：
        ![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/6073640f44044d698b0072f4774dc376.png)
    * `redo log 页结构`：
        * 用来存储`redo`日志的`页`称为`block`，大小为`512字节`
        ![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/4da09f66f6c308425ac517a60d88cb27.png)
        * log block body：存储redo log 记录
        * log block header、trailer：存储管理信息
    * `redo log buffer 结构`：
        ![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/0b35d4fd38effb23fb2b9f681a0b885f.png)
        ...
        * 一个事务可以有多个mtr，每当一个`mtr`执行完成时，伴随该`mtr`生成的一组`redo`日志就需要被复制到` redo log buffer`；多个事务的多个mtr可以交叉写入buffer；当事务提交时，需要将所有mtr对应的`redo log`刷盘。
        ![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/7478dff968b6837d49ca55b320f5576d.png)

#### redo log在磁盘中的结构：
* redo日志文件组：
    * 文件名称：ib_logfile_x
    * 大小：每个redo log文件大小：innodb_log_file_size 默认48MB,每组文件数量 innodb_log_files_in_group 默认2
    ![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/8cb7009677c3b76ad185111ddedeaf4e.png)
* redo log 刷盘：
...
* `flush链表中的lsn`：
    ...
    * `newest_modification`：某个页每次被修改，都会将修改该页的mtr结束时对应的lsn更新到newest_modification
    ![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/0465825ba6f983b02e487aba59e0a7fe.png)
    * 上图解释：一次事务中，发生三次mtr
...
* `checkpoint_lsn`：
    ...
    * checkpoint过程：
        ...
        * 步骤二：更新checkpoint_no（每次checkpoint加1）、checkpoint_lsn（最早的脏页o_m）、checkpoint_offset(lsn在日志文件中的offset)到redo日志文件组的管理信息中（当checkpoint_no的值是偶数时写到checkpoint1中，是奇数时写到checkpoint2中）
        ![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/45bfe5c9510dc4b39bf16f5ed1ba4e83.png)
...
* 各种lsn和redo log file的关系：
    ...
    * checkpoint_lsn: `flush 链表`中未刷入磁盘中最早的脏页对应的lsn
    ![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/dba504c794857f9b872544c3fc7a8dc2.png)
* 查看系统的各种lsn：
* ``` 
...
#### undo log和数据页的关系：
* 通过roll_pointer关联
![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/7a54bb736234d0de9f4123f098aa3b63.png)
...
* 多个事务中的Undo页面链表：
    * 为了尽可能提高`undo日志`的写入效率，不同事务执行过程中产生的undo日志需要被写入到不同的Undo页面链表中，这样每个事务的链表完全独立
    ![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/82518c9575b52a222af05efb30dcf676.png)
* 事务写undo log的过程：
    * 一个事务在向Undo页面中写入undo日志时的方式是十分简单暴力的，就是直接往里怼，写完一条紧接着写另一条，各条undo日志之间是亲密无间的。写完一个Undo页面后，再从段里申请一个新页面，然后把这个页面插入到Undo页面链表中，继续往这个新申请的页面中写。
    ![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/5be8fe71f51b3e3137616de73544a694.png)
...
* 回滚段存储在系统表的5号页面：
* 该页面可以存128个回滚段
![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/9d70f85b277c60605759bc59ff9cc61c.png)
