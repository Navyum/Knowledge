## HTTP
#### 发送格式：
![图片](./IMG/8.%20应用层（七层）.md/2b1e9ffb.png)
...
* 举例：TLS_AES_256_GCM_SHA384（TLS 1.3）
    ...
    * 签名算法不在套件内，而是通过`signature_algorithms`扩展指定
    ![Img](./IMG/8.%20应用层（七层）.md/img-20240530110022.png)

#### TLS 1.2 （ECDHE）握手过程：
![Img](./IMG/8.%20应用层（七层）.md/img-20240529162515.png)
...
#### TLS 1.2 和 1.3对比：
![Img](./IMG/8.%20应用层（七层）.md/img-20240529161022.png)
...
- Huffman 编码（压缩算法）
![Img](./IMG/8.%20应用层（七层）.md/img-20240530143740.png)
- 存在的问题：
...
![Img](./IMG/8.%20应用层（七层）.md/img-20240530144142.png)
...
* `h2`包解析：
![Img](./IMG/8.%20应用层（七层）.md/img-20240530190841.png)

##### `多路复用`：
* 含义：
    * 一条 TCP 连接在同一时间可以跑多个Stream
* TCP connection、stream、message、frame的关系：
![Img](./IMG/8.%20应用层（七层）.md/img-20240530151232.png)
...
#### 帧协议格式：
![图片](./IMG/8.%20应用层（七层）.md/075d9fbc.png)
  ...
  Packet Header                   -----
      QUIC Frame Header           --  TLS1.3
          HTTP3 Frame Header      -- 
              HTTP Message        -----
```
![Img](./IMG/8.%20!应用层、HTTP（七层）.md/img-20240602184626.png)
...
* Packet（负责连接可靠）：
    ...
    * 一个 Packet 报文中可以存放多个 `QUIC Frame`，不能超过PMTUD（1200字节）
    ![Img](./IMG/8.%20应用层（七层）.md/img-20240531160644.png)
...
* QUIC Frame（负责字节流有序）：
    ...
    * `Frame Type`：
        * 定义不同类型的Frame
            ...
            * `other Frame` 状态管理
    ![Frame通用头](./IMG/8.%20应用层（七层）.md/img-20240531170056.png)
    * `QUIC STREAM Frame`
        * `Stream Id` 实现有序的字节流：
        ...
        * 以下为QUIC STREAM Frame格式：
        ...
        ![Stream Frame](./IMG/8.%20应用层（七层）.md/img-20240531165600.png)

* HTTP/3 Frame（负责HTTP语义）：
    ![HTTP3 通用Frame头信息](./IMG/8.%20应用层（七层）.md/img-20240531170910.png)
    * `Type`：类似 HTTP/2的帧类型：
...
* stream级别的滑动窗口：
    ...
    窗口示意图：TCP 和 QUIC
        ![图片](./IMG/8.%20应用层（七层）.md/f0e09620.png)
    5. 可用窗口左边界：
        * TCP：`已确认的`序号
        * QUIC：`收到的`最大Offset
* 连接级别的流量控制窗口：
...
* QPACK：
    ...
    * QPACK使用两个额外的单向QUIC流，用于在两个方向上`传递动态表信息`
        * `QPACK Encoder Stream`：用于传输动态表信息
        ![Img](./IMG/8.%20!应用层、HTTP（七层）.md/img-20240602185022.png)

        * `QPACK Decoder Stream`：用于对传输内容的确认响应
        ![Img](./IMG/8.%20!应用层、HTTP（七层）.md/img-20240602185100.png)


##### `二进制格式编码`：
* 同HTTP2.0相似，将传输信息（header+body）分割为更小的帧
![Img](./IMG/8.%20!应用层、HTTP（七层）.md/img-20240602184604.png)


##### HTTP/3.0 Frame：
![Img](./IMG/8.%20应用层（七层）.md/img-20240531154334.png)
* 帧类型与HTTP2基本相同
* 流的相关功能前置到传输层QUIC实现
* 头部压缩算法使用QPACK
