...
![Img](./IMG/01.%20数据类型和底层数据结构.md/img-20240520192328.png)
...
* embstr 和 raw的区别：
    * embstr编码时，只进行一次内存分配，`redisObject`和[`SDS`](#简单动态字符串-`SDS`：)结构存储在一块连续内存。一般用于查，如果对embstr编码的数据做修改，会先改成raw再改
...
#### 特征：
* 内存占用小且连续，分配速度快，随机访问相对高效，插入删除相对低效
...
* `prevlen`的作用：
    * 快速方向遍历：通过记录前一个 entry 的长度，可以快速定位到前一个元素的起始位置
* encoding:
...
2. 查找其他元素时，只能逐个查找，此时的复杂度就是 O(N)。如果 ziplist 里面保存的是字符串，ziplist在查找某个元素时，还需要逐个解码元素，判断每个元素的data大小。
...
### hashtable 哈希表：
![Img](./IMG/01.%20数据类型和底层数据结构.md/img-20240521120145.png)
#### 特征：
