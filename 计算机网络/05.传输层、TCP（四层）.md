...
### 协议格式：
![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/63dc99d734ef85b68c95f6d4a1645777.png)
...
- TCP用四个元组来表示是同一个连接
![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/297de65e785b33533c0f4abe533e5dce.png)
...
### TCP 协议格式：
![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/05ecb9399c2b6a816b58b3fd1e890275.png)
...
#### 1. 维护连接状态：
* TCP状态机：
  ![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/84fe5f77d405144573c34a93c0ab95ae.png)
* TCP 握手、挥手过程在内核协议栈进行,TLS在应用层
* 三次握手：
  ![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/9cbbbdb61b7ed570a2d1c7e93aa6c8c5.png)
  * 握手为什么至少三次：
    * 原因一：避免历史连接
    * 原因二：需要同步`双方`初始序列号
    * 原因三：避免资源浪费
      * 四次握手的浪费：
        ![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/e056331af380dc8746a9d97ef09514e8.png)
* 四次挥手：
  ![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/91ea2f98e977d712f5a896d15e6d0820.png)
  ...
  * TIME_WAIT
    ...
    * 四次挥手变成三次挥手
      * [TCP 延迟确认机制](#TCP-延迟确认机制：)
        ![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/b3baa53dc153b76a064a2ef829ca21c8.png)
    * 挥手特殊说明：
      * 挥手过程在内核协议栈进行
        * 机器宕机，无法正常挥手
        * 进程崩溃，正常挥手
...
* **重传机制**：
    ...
    * `2.快速重传`（Fast Retransmit）：
        * 定义：`以数据驱动重传`。接收端因为数据没有收到，会重复ACK可能丢失的包（累计确认），发送端连续收到`3次`就重传。
        ![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/4705095d72f9876cc007e13c3e14b870.png)
        * 过程：
    ...
    * 重传发生时，应该重传哪些数据：
        ...
        * `SACK` 选择性确认：
            * `接收方`额外返回`已收到的数据区间`，让发送端有选择的重传，而不必重传已收到的包
            ![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/37e2287f8d81753409afe59471b47ed1.png)
        ...
        * `D-SACK` Duplicate SACK：
            ...
            * `DSACK` 可以让发送方知道：
                ...
                * 网络上是否把包做了复制
            ![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/6603050de3bd575c51dfea5b58726dc3.png)
...
* `滑动窗口` Sliding-Window：
    ...
    * 发送方滑动窗口示意图：
        - 向右滑动 👉👉
        ![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/17c8fe857cbb66f044889e93b8c80a30.png)

        - 可用窗口计算：![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/f2674754fab25bef42d72bd52b704620.png)
    ...
    * 接收方滑动窗口示意图：
        * 计算：![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/471844fdbdf583d87b205c246c7ca98d.png)
            - `RCV.WND`：`接收窗口的大小`
...
* 拥塞控制主要过程：
    * `慢启动门限 ssthresh` （slow start threshold）
    ...
    3. `拥塞发生`：
        ...
        * `基于重传类型分为两种`：
            * 如果出现`超时重传`：
              ![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/803c2581f2a5a16c657cf6e0bf1394a2.png)
        ...
        * `TCP New Reno 快速恢复`：
            ...
            * 过程：
                ...
                2. 判断重传的包返回的Ack信息能否覆盖发送端已发送未确认的所有数据
                    ...
                    1. 如果可以覆盖，则快速恢复结束
            * 示意图：
                ![图片](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/efaea203fecccf29701f935fabd7587d.png)
        * FACK 算法：
...
- 图例：
![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/73569d19d8f0f6878b794cff79d1c009.png)
- 底层结构：
    - 全连接队列：链表
    - 半连接队列：哈希表、hashkey(sourceip+port)
    ![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/2308af33cd202b8b7b7606801a6ea791.png)
- 查看队列大小：
    - 全连接：ss -lnt
    - 半连接：netstat -nt | grep -i '127.0.0.1:8080' | grep -i 'SYN_RECV' | wc -l
    
### TCP SYN攻击：
![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/323dd299d0cf90461b7ce78680846b36.png)
* 说明：
...
* 具体策略：
    ...
    * 但是如果在延迟等待发送 ACK 期间，对方的第二个数据报文又到达了，这时就会`立刻发送 ACK`
    ![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/8ff282c7a435e1c4dfd6ae3a8adae2ce.png)

* Nagle 算法

### TCP 累计确认机制：
![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/186447b1ed6b4c95d7b0b9d9d71b681d.png)
...
* 具体流程：
    ...
    * 客户端收到这个ACK，发现确认号并不是自己期望收到的，于是回复`RST报文`关闭连接
    ![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/55ec46c05fcc294f67733f271098d64a.png)
* 实际应用：
    * killcx：
