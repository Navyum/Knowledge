...
## HTTP
#### 发送格式：
![图片](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/3e072d70ba9af070642f25856b372028.png)

#### 响应格式：
![图片](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/31bc1a1c077cb1ef967ba20d2cda4c55.png)
...
* 举例：TLS_AES_256_GCM_SHA384（TLS 1.3）
    ...
    * 签名算法不在套件内，而是通过`signature_algorithms`扩展指定
    ![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/6e438da1013944db557141a57deb690b.png)

#### TLS 1.2 （ECDHE）握手过程：
![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/8e1a73d1c9158a0ec1040f1777f6019e.png)
...
#### TLS 1.2 和 1.3对比：
![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/1277641be6ab34323df77c4f02950142.png)
...
- Huffman 编码（压缩算法）
![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/c4803aa491fcfd5ebfde9045d606c748.png)
- 存在的问题：
...
![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/6d7699ba78eba5dae358267adbf382e8.png)
...
* `h2`包解析：
![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/c42e845055af9f5a4ceb78dafd14087a.png)

##### `多路复用`：
* 含义：
    * 一条 TCP 连接在同一时间可以跑多个Stream
* TCP connection、stream、message、frame的关系：
![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/22170a9e46dad8ff73adff878d2ede0a.png)
...
#### 帧协议格式：
![图片](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/048a9b0325f365cd4f0bc626fa106ebf.png)
...
* 从不同层对应的消息：
    ...
    * 在“TCP”的层面上看，消息是乱序收发的“帧”
    ...
    ![图片](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/8fae57c8dcf3faf0f41577042bd9ec58.png)
    ![图片](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/7ddf8f4fadc857fe081fd2008950a987.png)
* 数据发送过程：
    ...
    6. 1.1 vs 2.0 发送示意图：
    ![图片](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/0e49e74abc6c7694352d3642d59bdb5d.png)
  ...
  Packet Header                   -----
      QUIC Frame Header           --  TLS1.3
          HTTP3 Frame Header      -- 
              HTTP Message        -----
```
![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/1e1b1cd669b4881acee9400ff91bb680.png)
...
* Packet（负责连接可靠）：
    ...
    * 一个 Packet 报文中可以存放多个 `QUIC Frame`，不能超过PMTUD（1200字节）
    ![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/f443e834f43f34c9e2e0647d29b8c24c.png)
...
* QUIC Frame（负责字节流有序）：
    ...
    * `Frame Type`：
        * 定义不同类型的Frame
            ...
            * `other Frame` 状态管理
    ![Frame通用头](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/c33058203fdba04e7b5337171eaf8756.png)
    * `QUIC STREAM Frame`
        * `Stream Id` 实现有序的字节流：
        ...
        * 以下为QUIC STREAM Frame格式：
        ...
        ![Stream Frame](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/7569ef8b54999da315b58a34914cf392.png)

* HTTP/3 Frame（负责HTTP语义）：
    ![HTTP3 通用Frame头信息](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/d1d2180748b4f6e04c42da9e54754b2a.png)
    * `Type`：类似 HTTP/2的帧类型：
...
* stream级别的滑动窗口：
    ...
    窗口示意图：TCP 和 QUIC
        ![图片](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/6fda22d5782f5f61402846f9e21d018a.png)
    5. 可用窗口左边界：
        * TCP：`已确认的`序号
        * QUIC：`收到的`最大Offset
* 连接级别的流量控制窗口：
...
* QPACK：
    ...
    * QPACK使用两个额外的单向QUIC流，用于在两个方向上`传递动态表信息`
        * `QPACK Encoder Stream`：用于传输动态表信息
        ![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/f02bf0f32626d8a7f3e43ecb3efc4291.png)

        * `QPACK Decoder Stream`：用于对传输内容的确认响应
        ![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/d5a20c3165d8a69314170350c88d2dab.png)
...
##### HTTP/3.0 Frame：
![Img](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/386bc276ab8dd65901f640bd19e7a20d.png)
* 帧类型与HTTP2基本相同
* 流的相关功能前置到传输层QUIC实现
* 头部压缩算法使用QPACK
...
## 对比：
![图片](https://cdn.jsdelivr.net/gh/Navyum/imgbed@pic/IMG/66735265a54c7ed385a2a94b88e4cda4.png)
* 参考：
