...
|---|---|---|---|---|
| 不可变状态 + COW 分支（Delta 状态树） | 函数式/文件系统 Copy-on-Write | LangGraph checkpoint | ◐ | [CH8](../08-状态持久化/ch8-state.md) |
| 结构感知截断（保留头部签名 + 尾部异常） | 编译器 AST / 词法分析 | Claude Code、Aider | ◐ | [CH4](../04-工具系统/ch4-tools-claude-code.md) |
| KV-Cache 友好的增量摘要 | 注意力键值缓存复用 | Claude Code compact、AutoGPT | ✅ | [CH5.3](../05-上下文/ch5-ctx-compact.md) |
| 上下文分页 / Off-loading（外部记忆） | OS 虚拟内存 / Paging | MemGPT / Letta | ◐ | [CH5.1](../05-上下文/ch5-ctx-principles.md) |

**② 运行时与沙箱**（启动速度 vs 隔离强度）

| 关键技术 | 借自 | 代表选型 | 级别 | 课程位置 |
|---|---|---|---|---|
| OverlayFS 增量快照（毫秒级回滚） | Linux 联合文件系统 | SWE-agent、Devin 类 | ◐ | [CH7 沙箱光谱](../07-运行时/ch7-rt-principles.md) |
| MicroVM（Firecracker/KVM） | 轻量级硬件虚拟化 | E2B、Modal | ◐ | [CH7 沙箱光谱](../07-运行时/ch7-rt-principles.md) |
| Namespaces + Cgroups + Seccomp | Linux 容器三件套 | OpenHands、Docker 沙箱 | ✅ | [CH7](../07-运行时/ch7-rt-principles.md) |
| eBPF 运行时监控（无侵入挂钩） | Linux 内核可观测 | 企业审计层 | ○ | [CH7 沙箱光谱](../07-运行时/ch7-rt-principles.md) · [CH10](../10-可观测/ch10-observability.md) |

**③ 权限与安全**（LLM 不确定性 vs 执行确定性）

| 关键技术 | 借自 | 代表选型 | 级别 | 课程位置 |
|---|---|---|---|---|
| 命令 AST 静态解析（非正则黑名单） | 编译器 Lexer/Parser | Claude Code BashTool | ✅ | [CH4](../04-工具系统/ch4-tools-claude-code.md) · [CH6](../06-权限安全/ch6-perm-principles.md) |
| 出站代理 + SNI 域名白名单 | 网络透明代理 / TLS 检查 | E2B、K8s NetworkPolicy | ◐ | [CH6 注入防御](../06-权限安全/ch6-perm-principles.md) |
| 路径规范化 + chroot/Landlock（防目录穿越） | OS 文件系统安全 | SWE-agent | ◐ | [CH6](../06-权限安全/ch6-perm-principles.md) |
| 能力基安全 / Macaroons（带时效范围的强类型 token） | 能力安全模型 | MCP 权限设计**受此启发** | ○ | [CH4 MCP](../04-工具系统/ch4-tools-aci.md) |

**④ 工具系统**（简单 Schema vs 复杂真实交互）

| 关键技术 | 借自 | 代表选型 | 级别 | 课程位置 |
|---|---|---|---|---|
| PTY 伪终端仿真（Stateful Shell） | Unix 伪终端 | Claude Code、OpenHands | ✅ | [CH4](../04-工具系统/ch4-tools-claude-code.md) · [CH7](../07-运行时/ch7-rt-openhands.md) |
| JSON Schema JIT 校验（Pydantic/Zod） | 强类型运行时校验 | LangChain、LlamaIndex | ◐ | [CH9 畸形 tool call](../09-错误处理/ch9-errors.md) |
| DAG 并行拓扑调度 | 分布式任务编排 | Semantic Kernel、CrewAI | ◐ | [CH12](../12-多agent/ch12-multi-agent.md) |
| 增量流式解析 + 背压 | 流式 SSE / 部分 JSON 解析 | Vercel AI SDK、Claude CLI | ✅ | [CH3](../03-模型交互/ch3-io-streaming.md) |

**⑤ 循环与调度**（死循环风险 vs 可重放性）

| 关键技术 | 借自 | 代表选型 | 级别 | 课程位置 |
|---|---|---|---|---|
| Event Sourcing（事件溯源，100% 重放） | 分布式系统事件溯源 | OpenHands EventStream、LangGraph | ✅ | [CH2](../02-agent循环/ch2-loop-openhands.md) · [CH8](../08-状态持久化/ch8-state.md) |
| 熔断器 + 环路检测（Hash/语义查重） | 微服务 Circuit Breaker | Claude Code QueryGuard | ✅ | [CH9](../09-错误处理/ch9-errors.md) |
| FSM / 状态图约束（Conditional Edges） | 有限状态自动机 | LangGraph、MetaGPT | ◐ | [CH2](../02-agent循环/ch2-loop-paradigms.md) |
| Checkpoint/Resume（WAL 预写日志思想） | 数据库 Write-Ahead Logging | LangGraph Checkpointers | ✅ | [CH8](../08-状态持久化/ch8-state.md) |
