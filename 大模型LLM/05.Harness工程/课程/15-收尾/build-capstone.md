...
> **这个结构不是巧合**
>
> 对照 [Claude Code 案例](../14-案例深剖/case-claude-code.md)的模块清单：`model.py`≈`services/api/`、`context.py`≈`services/compact/`、`permission.py`≈`hooks/toolPermission/`、`session.py`≈`history/`……**你的 tinyharness 和真实 harness 的模块划分是同构的**，只是每块更小。这证明你掌握的是骨架，不是玩具。

## 总装后的循环：所有部件如何咬合

主循环 `harness.py` 把九块串起来。这就是 [任务地图](../01-基础/lifecycle.md)那张图的代码化身：
