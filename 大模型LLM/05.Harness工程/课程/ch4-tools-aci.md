...
> **回到任务**：[任务地图](lifecycle.md)第 ⑦、⑨ 步："执行工具""模型决定写测试"。`parseDate` 任务里 agent 要读文件、写测试、跑 pytest——这些能力全靠工具提供。工具设计得好不好,直接决定 agent 能不能高效完成任务。这一章就讲怎么设计好工具。
...
*工具是执行链的末端:上游是权限放行、运行时提供的受控环境，工具在其中真正"动手"；执行完的结果回写进上下文，闭合 ReAct 的 observe 环节。它是 agent 触达外界的末端（[全景协作图见 intro](intro.md)）。*
...
内置工具是你自己写的，MCP server 往往是第三方进程。这带来三重新风险（详见 [CH6](ch6-perm-principles.md)）：
...
| 重试 | 简单重试即可 | 需指数退避 + 幂等考量（呼应 [CH3.2](ch3-io-reliability.md)） |
| 超时 | 短超时够用 | 要更宽松超时 + **熔断**（呼应 [CH13](ch13-production.md)） |
