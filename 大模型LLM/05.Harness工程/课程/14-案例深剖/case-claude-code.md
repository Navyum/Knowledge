...
**关于本页源码**：基于 Claude Code 泄露 source map 还原（来源与合规边界见[附录 C](../15-收尾/appendix.md)）。只讲架构、模块职责与文件定位，图为原创重绘，不复制真实源码。
...
> **说明：Claude Code 闭源，无官方架构图。** Claude Code 是闭源产品，官方未公开整体架构图。下方架构图是本课基于泄露 source map 还原的源码目录结构**原创绘制**（来源与合规边界见[附录 C](../15-收尾/appendix.md)），只表达模块划分与调度关系，不代表官方设计文档。
