...
> **关于本节源码**：基于 Claude Code 泄露 source map 还原（来源见[附录 C](../15-收尾/appendix.md)）。只讲架构、文件职责与设计思路,代码为原创伪代码示意,不复制真实源码。

> **回到任务**：[任务地图](../01-基础/lifecycle.md)第 ⑩ 步"跑测试（危险操作）"。`parseDate` 任务要跑 `pytest`,靠的就是 BashTool。而"跑任意命令"是 agent 最危险的能力——本节你会看到 Claude Code 为这一个工具堆了多少安全设计。
