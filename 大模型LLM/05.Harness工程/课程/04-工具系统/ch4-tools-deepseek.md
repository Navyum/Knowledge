...
> **回到任务**：还是[任务地图](../01-基础/lifecycle.md)第 ⑥/⑦ 步"权限检查 → 执行工具"。上一节看了 Claude Code 怎么组织工具,这一节看同一件事的另一种架构——理解"工具系统还能怎么设计",你才能为自己的场景做对选择。
...
> **作用域 vs 事后拒绝**
>
> 注意差别：一个只读 agent**压根看不到**写工具,而不是"看到了但执行时被拒"。这比"先给再拒"更安全、也更省 token（模型不会浪费 token 去尝试一个注定被拒的工具）。这对多 agent 场景（[CH12](../12-多agent/ch12-multi-agent.md)）尤其重要——给子 agent 精确的工具子集,是隔离的一部分。
