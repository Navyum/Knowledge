...
| 维度 | 一极 | 另一极 | 选择依据 |
|---|---|---|---|
| [循环架构](ch2-loop-paradigms.md) | 生成器内联 | 事件驱动外置 | 是否需持久化/分布式/接管 |
| [工具复杂度](ch4-tools-aci.md) | 富工具集 + 强 ACI | bash-only 极简 | 目标模型能力（越强越可简） |
| [上下文策略](ch5-ctx-principles.md) | 多技法叠加 | 全量塞入 | 任务长度/成本敏感度 |
| [执行后端](ch7-rt-principles.md) | 容器/远程隔离 | 进程内/本地 | 安全需求 vs 启动开销 |
| [权限模型](ch6-perm-principles.md) | 沙箱为主 | 审批/git 兜底 | 自动化 vs 交互；是否可信 |
| [模型绑定](ch11-model-binding.md) | 绑定单模型 | provider 中立 | 优化深度 vs 迁移自由 |
| [多 agent](ch12-multi-agent.md) | 并行 subagent | 单线程收敛 | 读为主 vs 写为主 |
...
| 接缝 | 谁给谁 | 传递什么 | 出错会怎样 |
|---|---|---|---|
| 循环 → 权限 | [CH2](ch2-loop-paradigms.md)→[CH6](ch6-perm-principles.md) | 解析出的 tool_use（工具名+参数） | 漏了这道闸 = agent 无审批直接跑危险命令 |
| 权限 → 运行时 | [CH6](ch6-perm-principles.md)→[CH7](ch7-rt-principles.md) | 放行的动作 + 该在哪个隔离级执行 | 权限放行但运行时没隔离 = 沙箱形同虚设 |
| 工具 → 上下文 | [CH4](ch4-tools-aci.md)→[CH5](ch5-ctx-principles.md) | 执行结果 / 格式化后的错误 | 结果格式化差 = 模型读不懂、无法自纠（[CH9](ch9-errors.md)） |
| 状态 ↔ 全体 | [CH8](ch8-state.md) 横切 | 每个事件的 append-only 记录 | 不记录 = 崩溃后无法恢复、无法回放审计 |
