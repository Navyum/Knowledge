...
前置:Python 3.10+,一个能用的模型 API(示例用 Anthropic Messages API 的伪封装 `call_model`,你可换成任意 provider——[CH3](ch3-io-streaming.md) 会讲真实的流式调用,本节先用非流式把骨架跑通)。

## 第 1 步:定义消息与模型调用

**消息数组 + 模型封装**

harness 的"记忆"就是一个 messages 列表([CH5](ch5-ctx-principles.md) 会深挖它)。模型调用先做成一个函数,输入 messages 和 tools,返回模型响应。
...
一个工具有两半:给模型看的 **schema**(它据此决定何时调、传什么参数),和真正执行的 **函数**。[CH4](ch4-tools-aci.md) 会讲工具设计的 ACI 原则,这里先做最简单的。
...
| --- | --- | --- |
| 非流式,等整条响应 | 流式收流,增量解析 tool_use | [CH3](ch3-io-streaming.md) |
| API 挂了就崩 | withRetry / 限流退避 / 缓存 | [CH3.2](ch3-io-reliability.md) |
| 只有 read_file | 42 个工具 + MCP + 安全护栏 | [CH4](ch4-tools-aci.md) |
| 无 system prompt 组装 | 系统提示 + 工具 schema + 记忆拼装 | [CH5.2](ch5-ctx-systemprompt.md) |
| messages 无限增长会爆 | 近上限自动 compact | [CH5.3](ch5-ctx-compact.md) |
| 工具直接执行,无审批 | 权限闸 + 沙箱 | [CH6](ch6-perm-principles.md) |
| `while True` 可能空转 | token 预算 + 停止钩子 + 循环卫生 | [CH9](ch9-errors.md) |
...
>
> 本节的最小循环只做到 ②(靠"无 tool_use"停),**不等于** ③。真正的"完成"判定要靠独立验收——这正是 [CH10 自建评测](ch10-observability.md)要补的:用预置测试客观判定,而非信模型自述。
