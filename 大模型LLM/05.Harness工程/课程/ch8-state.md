...
> **关于本节源码**：基于 Claude Code 泄露 source map 还原（来源见[附录 C](appendix.md)）。只讲架构与文件职责，代码为原创伪代码示意。

> **回到任务**：[任务地图](lifecycle.md)贯穿全程。`parseDate` 任务跑到一半你想去吃个饭、或网断了、或不小心关了终端——回来还能接着跑吗？生产级 harness 的答案是"能"。这靠的就是本章的持久化与恢复。
...
不是所有状态都值得落库。全落库排查方便但成本高、拖慢吞吐（[CH13](ch13-production.md) 的"写放大"瓶颈）；全内存快但重启即丢、并发易 OOM（Out Of Memory，内存耗尽）。生产做法是**按"要不要恢复 + 要不要排查"两个问题分层**：
